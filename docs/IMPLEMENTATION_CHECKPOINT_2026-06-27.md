...
| 9. Frontend vertical slices | Complete | Generated OpenAPI types, React Hook Form/Zod, 35-airport autocomplete, six selectable aircraft types, TanStack Query, deterministic results, objective and fuel breakdown, and Storybook exist. Route views use MapLibre GL over real OpenStreetMap cartography with visible attribution, backend GeoJSON, great-circle interpolation between authoritative solver nodes, antimeridian-safe lines/camera bounds, airport-code and synthetic-node markers, fit-to-route navigation, configurable deployment style and an explicit unavailable-basemap state; the former hand-drawn continents and synthetic wind field were removed. Wind summaries remain derived from API-provided route-node components, while a separately controlled heatmap and directional-arrow overlay consumes the real 40-point Open-Meteo corridor field and reports unavailable data explicitly. React Testing Library/MSW covers widebody submission and degraded behavior; CI checks generated-client freshness against `aeroroute-contracts`; six mocked Playwright journeys cover happy/degraded/MLX/history/accessibility behavior; and a no-mock B788 journey passes against the live API and PostGIS. | Keep mocked browser tests in PR CI, run the real-stack journey in release verification, use an operated tile/style provider outside low-volume local development, and replace the fixed MVP corridor with viewport-aware forecast tiling before broader route support. |
...
| `aeroroute-api` | Ruff, 37 collected tests including real PostGIS migration/import/lifecycle/explanation coverage, weather forecast/fallback orchestration, wind-field pressure selection, idempotency, concurrency/deadline guards, stable errors and CORS preflight, Alembic SQL through revision 0005, sdist and wheel passed. |
...
Total automated tests observed: 129.
