...
| 2. Database and airport catalogue | Complete | PostGIS schema, Alembic migrations, immutable bundle generation/import, airport search and a real-PostGIS integration test cover upgrade/downgrade, idempotency and spatial coordinate order. A reproducible public-domain OurAirports MVP bundle provides 35 globally distributed ICAO airports. Winner routes are enriched through AIRAC.net with nearby named C/R/W navigation fixes, AIRAC cycle, region and snap distance; unresolved oceanic points retain explicit coordinate names. Five candidates per solver layer are evaluated with dynamic programming to prefer shared airway membership, then consecutive fixes are audited against the AIRAC airway-route endpoint and tagged with confirmed designators or `DCT`. DXB-MAD demonstrates that coarse solver layers still require a local multi-airway segment graph to insert intermediate intersections. | Ingest a versioned airway-segment graph and run bounded path search, then add procedure/runway selection before claiming a connected or fileable flight plan. |
...
| 9. Frontend vertical slices | Complete | Generated OpenAPI types, React Hook Form/Zod, 35-airport autocomplete, six selectable aircraft types, TanStack Query, deterministic results, objective and fuel breakdown, and Storybook exist. Route views use MapLibre GL over real OpenStreetMap cartography with visible attribution, backend GeoJSON, great-circle interpolation, antimeridian-safe lines/camera bounds, airport and navigation-point markers, fit-to-route navigation, configurable deployment style and an explicit unavailable-basemap state. AIRAC fixes, oceanic coordinates and solver fallbacks are visually and semantically distinct; the technical navlog exposes cycle, snap distance, confirmed inbound airway or `DCT`. Wind summaries remain derived from API-provided route-node components, while a separately controlled heatmap and directional-arrow overlay requests a real 40-point Open-Meteo field around each calculated route. React Testing Library/MSW covers widebody submission and degraded behavior; CI checks generated-client freshness against `aeroroute-contracts`; six mocked Playwright journeys and one real-stack journey pass. | Improve route construction to favor connected airways and add SID/STAR selection before presenting the sequence as a flight plan; retain the non-operational warning. |
...
Total automated tests observed: 134.
