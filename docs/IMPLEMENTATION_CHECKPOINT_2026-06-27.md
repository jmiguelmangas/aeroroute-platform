...
| 2. Database and airport catalogue | Complete | PostGIS schema, Alembic migrations, immutable bundle generation/import, airport search and a real-PostGIS integration test cover upgrade/downgrade, idempotency and spatial coordinate order. A reproducible public-domain OurAirports MVP bundle provides 35 globally distributed ICAO airports. Winner routes are enriched through AIRAC.net with nearby named C/R/W navigation fixes, AIRAC cycle, region and snap distance; unresolved oceanic points retain explicit coordinate names. Consecutive fixes are now audited against the AIRAC airway-route endpoint and tagged with confirmed airway designators or explicit `DCT`. | Improve fix selection using airway connectivity, then add procedure/runway selection before claiming a connected or fileable flight plan; keep the integration non-operational. |
...
| `aeroroute-api` | Ruff, 41 collected tests including real PostGIS migration/import/lifecycle/explanation coverage, AIRAC fix filtering/enrichment and airway-route parsing, weather forecast/fallback orchestration, dynamic wind-field corridor and pressure selection, idempotency, concurrency/deadline guards, stable errors and CORS preflight, Alembic SQL through revision 0005, sdist and wheel passed. |
...
Total automated tests observed: 133.
