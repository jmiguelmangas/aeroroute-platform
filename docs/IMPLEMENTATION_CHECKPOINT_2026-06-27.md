...
| 2. Database and airport catalogue | Complete | PostGIS schema, Alembic migrations, immutable bundle generation/import, airport search and a real-PostGIS integration test cover upgrade/downgrade, idempotency and spatial coordinate order. A reproducible public-domain OurAirports MVP bundle provides 35 globally distributed ICAO airports. The navigation adapter samples each route corridor, discovers nearby AIRAC.net C/R/W fixes and airway memberships, caches downloaded airway segments and procedures, builds a bounded local multi-airway graph and inserts intermediate intersections. Procedure exits are graph start nodes: DXB-MAD now resolves through SID `SENP2F` runway family `30B`, joins at `SENPA`, then follows a connected en-route graph to Spain; only the arrival join remains `DCT`. | Add explicit runway input and STAR-entry graph goals, persist cycle-versioned snapshots, and move first-load enrichment to progressive delivery before claiming a fileable flight plan. |
...
| 9. Frontend vertical slices | Complete | Generated OpenAPI types, React Hook Form/Zod, 35-airport autocomplete, six selectable aircraft types, TanStack Query, deterministic results, objective and fuel breakdown, and Storybook exist. Route views use MapLibre GL over real OpenStreetMap cartography with visible attribution, backend GeoJSON, great-circle interpolation, antimeridian-safe lines/camera bounds, airport and navigation-point markers, fit-to-route navigation, configurable deployment style and an explicit unavailable-basemap state. AIRAC fixes, SID points, oceanic coordinates and solver fallbacks are visually and semantically distinct; the technical navlog exposes cycle, procedure/runway family, confirmed inbound airway or `DCT`, and the map draws a separate cyan AIRAC projection over the green optimized trajectory. Wind summaries remain derived from API-provided route-node components, while a separately controlled heatmap and directional-arrow overlay requests a real 40-point Open-Meteo field around each calculated route. Six mocked Playwright journeys and one real-stack journey pass. | Add explicit runway input and STAR selection before presenting the sequence as a flight plan; retain the non-operational warning and progressively load cold AIRAC graphs. |
...
Total automated tests observed: 136.
